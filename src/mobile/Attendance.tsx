  ...
  return (
    <div className="p-6 flex flex-col items-center justify-center min-h-[70vh] space-y-12">
      <div className="text-center">
        <div className={`w-64 h-64 rounded-full border-4 flex items-center justify-center transition-all ${activeShift ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white shadow-xl'
          }`}>
          <button
            disabled={clocking || isScanning}
            onClick={handleStartScan}
            className={`w-52 h-52 rounded-full flex flex-col items-center justify-center gap-4 transition-all active:scale-90 shadow-2xl ${activeShift ? 'bg-emerald-600 shadow-emerald-500/30' : 'bg-slate-900 shadow-slate-900/40'
              }`}
          >
            {clocking ? (
