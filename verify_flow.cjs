...
async function verify() {
    ...
    const MGR_ID = '00000000-0000-0000-0000-000000000002'; // Layla Al-Fadhli
    const QUARTER = '2026-Q1';

    console.log('--- Step 1: Creating Evaluation ---');
    const { data: evaluation, error: e1 } = await supabase.from('employee_evaluations').insert([{
        employee_id: EMP_ID,
        evaluator_id: MGR_ID,
        quarter: QUARTER,
    ...
    await supabase.from('employee_evaluations').update({ status: 'APPROVED_FOR_PAYROLL' }).eq('id', evaluation.id);
    ...
    if (e4) {
        console.warn('Update failed:', e4.message);
    } else {
        console.log('Payroll Item updated with Performance Bonus.');
    }

    console.log('--- Step 5: Finalizing Run ---');
    ...
    console.log('Final Performance Bonus (DB):', item?.performance_bonus);
