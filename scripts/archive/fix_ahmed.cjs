...
async function run() {
    console.log("Updating Ahmed in app_users to 'Manager'...");
    const { error: e1 } = await stageClient.from('app_users').update({ role: 'Manager' }).eq('username', 'ahmed');
    console.log(e1 ? "ERROR: " + e1.message : "SUCCESS!");
}
run();
