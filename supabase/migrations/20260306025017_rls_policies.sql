    ...
    BEGIN
        ...
        LOOP
            ...
            EXECUTE format('DROP POLICY IF EXISTS "Everyone: Bootstrap Holidays" ON %I', t);
            EXECUTE format('DROP POLICY IF EXISTS "Everyone: Bootstrap Office" ON %I', t);
            EXECUTE format('DROP POLICY IF EXISTS "Everyone: Bootstrap Dept" ON %I', t);
            ...
            EXECUTE format('DROP POLICY IF EXISTS "Everyone: Read Permissions" ON %I', t);
            EXECUTE format('DROP POLICY IF EXISTS "Everyone: Read Templates" ON %I', t);
            EXECUTE format('DROP POLICY IF EXISTS "Admin: Manage Permissions" ON %I', t);
        END LOOP;
    END $$;
    ...
    -- BOOTSTRAP: Allow everyone to see the employee list count (needed for provisioning tool)
    CREATE POLICY "Everyone: Read Basic List" ON employees 
    ...
    -- BOOTSTRAP: Allow everyone to read and write (Needed for seeding tool before Auth is linked)
CREATE POLICY "Everyone: Bootstrap Access" ON employees FOR ALL TO anon, authenticated USING (true);
CREATE POLICY "Everyone: Bootstrap Holidays" ON public_holidays FOR ALL TO anon, authenticated USING (true);
CREATE POLICY "Everyone: Bootstrap Office" ON office_locations FOR ALL TO anon, authenticated USING (true);
CREATE POLICY "Everyone: Bootstrap Dept" ON departments FOR ALL TO anon, authenticated USING (true);
CREATE POLICY "Everyone: Bootstrap Users" ON app_users FOR ALL TO anon, authenticated USING (true);
CREATE POLICY "Everyone: Bootstrap Perms" ON role_permissions FOR ALL TO anon, authenticated USING (true);
CREATE POLICY "Everyone: Bootstrap Templates" ON permission_templates FOR ALL TO anon, authenticated USING (true);
