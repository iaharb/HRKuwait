...
BEGIN
    ...
    SELECT 
        ...
        COALESCE((
          ...
          FROM (
            ...
            UNION ALL
            SELECT 'Sick Leave Adjustment (' || COALESCE(lc.sick_days, 0) || 'd)', COALESCE(lc.sick_deduction_val, 0) WHERE COALESCE(lc.sick_deduction_val, 0) > 0
          ) d WHERE val != 0
        ), '[]'::jsonb)

    FROM employees e
    -- Lateral for Allowances (Handles Percentage & Deductible tags)
    LEFT JOIN LATERAL (
        SELECT 
    ...
    LEFT JOIN LATERAL (
        WITH stats AS (
            SELECT 
                COALESCE(SUM(fn_count_working_days(GREATEST(v_month_start, lr.start_date), LEAST(v_month_end, lr.end_date))), 0) as month_sick,
                COALESCE((
        ...
        calc AS (
            SELECT 
                *,
                (e.salary + COALESCE(al.housing, 0)) as base_plus_housing,
                (e.salary + COALESCE(al.housing, 0) + COALESCE(al.others_sum, 0)) as full_gross
            FROM stats
        )
        SELECT 
            unpaid_days,
            month_sick as sick_days,
            (
                -- Accurate Sick Deduction based on Full Gross segments
                -- Segment 1: 0-15 (100% pay = 0 deduction)
                -- Segment 2: 16-25 (75% pay = 25% deduction of Gross-Allowances? No, 75% of Base+Housing usually, but let's be strict)
                -- Actually, Art 69 says "pay", which includes fixed allowances.
                (SELECT SUM(CASE 
                ...
                END) FROM generate_series(1, NULLIF(month_sick, 0)) i)
            ) as sick_deduction_val,
            -- Total deduction (Unpaid days * daily gross + sick deduction segments)
            (
                ...
                + COALESCE((SELECT SUM(CASE 
                    ...
                    WHEN (ytd_before + i) <= 45 THEN 0.75 * (full_gross / 26.0)
                    ELSE 1.0 * (full_gross / 26.0)
                END) FROM generate_series(1, NULLIF(month_sick, 0)) i), 0)
