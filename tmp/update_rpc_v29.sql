-- Sentinel V29: The 'Flattened Supreme Sovereign'
-- 1. Fixed the "aggregate functions not allowed in FROM clause" error.
-- 2. Used scalar subqueries for sums to ensure Postgres compatibility.
-- 3. Maintained 26-Day Fixed Budget (February Fix).
...
BEGIN
    ...
    LEFT JOIN LATERAL (
        ...
        sick_sequence AS (
            SELECT 
                dt, l_type, is_paid,
                (SELECT COALESCE(SUM(fn_count_working_days(start_date, end_date)), 0) FROM leave_requests WHERE employee_id = e.id AND type='Sick' AND status IN ('Approved','Paid') AND start_date >= v_year_start AND start_date < v_month_start) +
                (SELECT COUNT(*) FROM month_days d2 WHERE d2.l_type='Sick' AND d2.dt <= d1.dt) as s_ord
            FROM month_days d1 WHERE kind='LEAVE' AND NOT is_paid
        )
        SELECT 
            (SELECT COUNT(*) FROM month_days WHERE kind='LEAVE' AND NOT is_paid) as ld_active,
            (SELECT COUNT(*) FROM month_days WHERE is_paid) as settled_days,
            -- Fixed Work Days
            GREATEST(0, 26 - ((SELECT COUNT(*) FROM month_days WHERE kind='LEAVE' AND NOT is_paid) + (SELECT COUNT(*) FROM month_days WHERE is_paid))) as wd_final,
            (GREATEST(0, 26 - ((SELECT COUNT(*) FROM month_days WHERE kind='LEAVE' AND NOT is_paid) + (SELECT COUNT(*) FROM month_days WHERE is_paid))) * (e.salary + COALESCE(al.a_s, 0)) / 26.0) as wp,
            -- Leave Sums
            COALESCE((SELECT SUM((e.salary / 26.0) * 
                CASE 
                    WHEN s.s_ord <= 15 THEN 1.0 
                    WHEN s.s_ord <= 25 THEN 0.75 
                    WHEN s.s_ord <= 35 THEN 0.50 
                    WHEN s.s_ord <= 45 THEN 0.25 
                    ELSE 0 END) 
            FROM sick_sequence s), 0) as lp_b,
            COALESCE((SELECT SUM(COALESCE(al.h_s, 0) / 26.0) FROM month_days WHERE kind='LEAVE' AND NOT is_paid), 0) as lp_h,
            -- PIFSS Shield
            EXISTS(
                SELECT 1 FROM payroll_runs pr JOIN payroll_items pi ON pr.id = pi.run_id
                WHERE pi.employee_id = e.id AND pr.status = 'Finalized'
